...
int	is_io_redirect(t_token_list **tok, t_parse_tree **redirect_node)
{
	...
	if (is_redirection_token((*tok)->token->type))
	{
		...
		*tok = (*tok)->next;
		if (*tok == NULL)
			return (free_parse_tree(*redirect_node), printf_global_error(2, 2, "my(s)hell: syntax error near unexpected token `newline'\n"), 1);
		if ((*tok)->token->type != WORD || *tok == NULL)
			return (free_parse_tree(*redirect_node), printf_global_error(2, 2, "my(s)hell: syntax error near unexpected token `%s'\n", (*tok)->token->lexeme), 1);
		(*redirect_node)->child = alloc_parse_tree();
		if ((*redirect_node)->child == NULL)
			return (free_parse_tree(*redirect_node), printf_global_error(2, 2, "my(s)hell: memory error\n"), 1);
